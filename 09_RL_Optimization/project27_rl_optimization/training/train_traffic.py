"""
Training script for Traffic Light Control Agent
...
from environments.traffic_light import TrafficLightEnv
...
def train_traffic_agent(
    ...
    num_envs: int = 4,
    seed: int = 42,
    save_dir: str = "models/traffic/",
    log_dir: str = "results/logs/traffic/",
    render: bool = False,
    verbose: int = 1
):
    """
    Train traffic light control agent.
    ...
    print(f"\n{'='*60}")
    print(f"Training Traffic Light Control Agent")
    print(f"{'='*60}")
    ...
    for episode in range(10):
        ...
        print(f"Episode {episode+1}: Reward={episode_reward:.2f}, "
              f"Avg Wait={env_info['average_wait_time']:.2f}s, "
              f"Vehicles={env_info['vehicles_passed']}, "
              f"Queue={env_info['total_queue_length']}")
    
    print(f"\nAverage Reward: {np.mean(episode_rewards):.2f} ± {np.std(episode_rewards):.2f}")
    print(f"Average Waiting Time: {np.mean([e['average_wait_time'] for e in episode_info_list]):.2f}s")
    print(f"Average Vehicles Passed: {np.mean([e['vehicles_passed'] for e in episode_info_list]):.1f}")
    print(f"{'='*60}\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train traffic light control RL agent"
    )
    parser.add_argument(
